...
class Veo3Service {
  constructor() {
    ...
    this.apiKey =
      "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNtcW5waGRpcG9tdXV3c3BsZmd2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDc5ODIzMzIsImV4cCI6MjA2MzU1ODMzMn0._p6qwLmco7JY6bWBHfouBIHCIHBHfUMgLyMNd-IGrS0";
  ...
  async enc(data) {
    const { uuid: jsonUuid } = await Encoder.enc({
      data: data,
      method: "combined",
    });
    return jsonUuid;
  ...
  async extractAuthDataFromCookie() {
    try {
      ...
      const authCookie = cookies.find(
        (cookie) => cookie.key === "sb-cmqnphdipomuuwsplfgv-auth-token"
      );
      if (!authCookie) {
  ...
  async createTempMail() {
    try {
      const response = await this.axiosInstance.get(
        `${this.baseTempMailUrl}?action=create`
      );
  ...
  async checkOTP() {
    ...
    try {
      const response = await this.axiosInstance.get(
        `${this.baseTempMailUrl}?action=message&email=${this.email}`
      );
  ...
  async requestOTP() {
    ...
    try {
      ...
      await this.cookieJar.setCookie(
        `sb-cmqnphdipomuuwsplfgv-auth-token-code-verifier=base64-"${Buffer.from(
          this.codeVerifier
        ).toString("base64")}"; Path=/; Domain=veo3.studio`,
        "https://veo3.studio"
      );
  ...
  async verifyOTP(otp) {
    ...
    try {
      const response = await this.axiosInstance.post(
        `${this.baseVeoUrl}/auth/verificationCode`,
        {
          email: this.email,
  ...
  async claimCredits() {
    ...
    try {
      ...
      const response = await this.axiosInstance.patch(
        `${this.supabaseRestUrl}/profiles?id=eq.${this.userId}`,
        {
          free_credits: 200,
        },
        {
      ...
      const response = await this.axiosInstance.post(
        ...
        requestData,
        {
          headers: {
            ...
            referer: "https://veo3.studio/create",
            "sec-ch-ua":
              '"Lemur";v="135", "", "", "Microsoft Edge Simulate";v="135"',
            "sec-ch-ua-mobile": "?1",
            "sec-ch-ua-platform": '"Android"',
            "sec-fetch-dest": "empty",
            "sec-fetch-mode": "cors",
  ...
  async status({ task_id }) {
    try {
      ...
      const { userid, cookies } = encryptData;
      ...
      const tempAxiosInstance = wrapper(
        axios.create({
          jar: tempCookieJar,
          withCredentials: true,
          timeout: 30_000,
      ...
      const response = await tempAxiosInstance.get(
        `${this.baseVeoUrl}/create/damo/recent-video?user_id=${userid}`,
        {
          headers: {
            priority: "u=1, i",
            referer: "https://veo3.studio/create",
...
export default async function handler(req, res) {
  // === CORS HEADERS (tambahan) ===
  // Ganti "*" dengan domain spesifik untuk production, contoh:
  // const ALLOWED_ORIGIN = "https://myfrontend.com";
    ...
    switch (action) {
      case "txt2vid":
        if (!params.prompt) {
          return res
            .status(400)
            .json({ error: "Prompt is required for txt2vid." });
        }
      ...
      case "status":
        if (!params.task_id) {
          return res
            .status(400)
            .json({ error: "task_id is required for status." });
        }
